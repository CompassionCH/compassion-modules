...
class easy_reconcile_advanced_bvr_ref(orm.TransientModel):
    ...
    def _skip_line(self, cr, uid, rec, move_line, context=None):
        ...
        if move_line.get('ref') and move_line.get('partner_id'):
            ...
            if credit_amount < total_due:
                # Check for other unreconciled credit lines that could complete the payment and reconcile all invoices.
                credit_lines = self._query_credit(cr, uid, rec, context=context)
                for other_credit in credit_lines:
                    if other_credit['ref'] == move_line['ref'] and not other_credit['id'] in rec_credit_ids:
    ...
    def _search_opposites(self, cr, uid, rec, move_line, opposite_move_lines, context=None):
        """
        Search the opposite move lines for a credit move line
        ...
        for op in opposite_move_lines:
            if amount_reconciled < move_line['credit']:
                if self._compare_opposite(cr, uid, rec, move_line, op, matchers, context=context) and not self._debit_reconciled(move_line, op):
    ...
    def _debit_reconciled(self, credit_line, debit_line):
        """ Test if a debit line is already fully reconciled. If not, update the amount reconciled by credit_line. """
