...
odoo.define("account_reconcile_compassion.reconciliation", function(require) {
  ...
  var reconciliation_renderer = require(
      "account.ReconciliationRenderer");
  var reconciliation_model = require("account.ReconciliationModel");
  var statement_action = require(
  "account.ReconciliationClientAction");
  ...
  statement_action.StatementAction.include({
      ...
      custom_events: _.extend({},
          statement_action.StatementAction.prototype
          .custom_events, {
      ...
      _onUpdatePropositionProgrammatically: function(event) {
          ...
          this.model.updateProposition(handle, event.data
              .data).then(function() {
              self._getWidget(handle).update(
              line);
          });
      },
      ...
      _openFirstLine: function(previous_handle) {
          ...
          var handle = _.compact(
              _.map(this.model.lines, function(line,
                  handle) {
                  // This is the FIX: !line.visible ||
                  return !line.visible ||
                      line.reconciled ||
                      parseInt(handle.substr(5)) <
                      parseInt(previous_handle
                          .substr(5)) ?
                      null :
                      handle;
              })
  ...
  reconciliation_renderer.StatementRenderer.include({
      events: _.extend({},
          reconciliation_renderer.StatementRenderer
          .prototype.events, {
      ...
      statementNameClickHandler: function() {
          this.do_action({
              ...
              target: "current",
              res_id: this.model.bank_statement_id
                  .id,
          });
      },
  });
  ...
  reconciliation_renderer.LineRenderer.include({
      ...
      _renderCreate: function(state) {
          ...
          return this.model
              ...
              .then(function(recordID) {
                  ...
                  var record = self.model.get(self
                      .handleCreateRecord);

                  self.fields.account_id =
                      new relational_fields
                      .FieldMany2One(
                          self,
                          "account_id",
                          record, {
                  ...
                  self.fields.force_tax_included =
                      new basic_fields.FieldBoolean(
                          self,
                          "force_tax_included",
                          record, {
                              mode: "edit"
                  ...
                  self.fields.label = new basic_fields
                      .FieldChar(
                          self,
                          "label",
                          record, {
                  ...
                  self.fields.date = new basic_fields
                      .FieldDate(self, "date",
                      record, {
                          mode: "edit",
                      });
                  ...
                  self.fields.avoid_thankyou_letter =
                      new basic_fields.FieldBoolean(
                          self,
                          "avoid_thankyou_letter",
                          record, {
                  ...
                  self.fields.account_id
                      .appendTo($create.find(
                          ".create_account_id .o_td_field"
                          ))
                      .then(addRequiredStyle.bind(
                          self, self.fields
                          .account_id));
                  self.fields.journal_id.appendTo(
                      $create.find(
                          ".create_journal_id .o_td_field"
                          )
                  );
                  self.fields.tax_ids.appendTo(
                      $create.find(
                          ".create_tax_id .o_td_field"
                          )
                  );
                  self.fields.analytic_account_id
                      .appendTo(
                          $create.find(
                              ".create_analytic_account_id .o_td_field"
                              )
                      );
                  self.fields.analytic_tag_ids
                      .appendTo(
                          $create.find(
                              ".create_analytic_tag_ids .o_td_field"
                              )
                      );
                  self.fields.force_tax_included
                      .appendTo(
                          $create.find(
                              ".create_force_tax_included .o_td_field"
                              )
                      );
                  self.fields.label
                      .appendTo($create.find(
                          ".create_label .o_td_field"
                          ))
                      .then(addRequiredStyle.bind(
                              self, self.fields.label
                              ));
                  self.fields.amount
                      .appendTo($create.find(
                          ".create_amount .o_td_field"
                          ))
                      .then(addRequiredStyle.bind(
                          self, self.fields.amount
                          ));
                  self.fields.date.appendTo($create
                      .find(
                          ".create_date .o_td_field"
                          ));
                  self.fields.to_check.appendTo(
                      $create.find(
                          ".create_to_check .o_td_field"
                          )
                  );
                  // CHANGE: Product, sponsorship, user_id and comment
                  // added from original function
                  self.fields.product_id.appendTo(
                      $create.find(
                          ".create_product_id .o_td_field"
                          )
                  );
                  self.fields.sponsorship_id.appendTo(
                      $create.find(
                          ".create_sponsorship_id .o_td_field"
                          )
                  );
                  self.fields.comment.appendTo(
                      $create.find(
                          ".create_comment .o_td_field"
                          )
                  );
                  self.fields.avoid_thankyou_letter
                      .appendTo(
                          $create.find(
                              ".create_avoid_thankyou_letter .o_td_field"
                              )
                      );
      ...
      update: function(state) {
          ...
          var sponsorship_field = this.fields
              .sponsorship_id;
  ...
  reconciliation_model.StatementModel.include({
      ...
      createProposition: function(handle) {
          ...
          if (gift_ref_key in gift_default_code) {
              var gift_ref_default_code =
                  gift_default_code[gift_ref_key];
          } else {
          ...
          if (gift_ref_default_code) {
              ...
              var gift_commitment_number =
                  gift_commitment_number_ref_key.replace(
                      /^0+/, '');
      ...
      updateProposition: function(handle, values) {
          ...
          if ("product_id" in values) {
              ...
              return this._rpc({
                  ...
                  return parent.call(self, handle,
                      values);
              });
          } else {
      ...
      quickCreateProposition: function(handle,
          reconcileModelId) {
          var line = this.getLine(handle);
          var reconcileModel = _.find(this
              .reconcileModels,
              function(r) {
          ...
          var focus = this._formatQuickCreate(line, _
              .pick(reconcileModel, fields));
          ...
          if (reconcileModel.has_second_line) {
              ...
              _.each(fields, function(key) {
                  second[key] =
                      "second_" + key in
                      reconcileModel ?
                      reconcileModel["second_" +
                          key] :
                      reconcileModel[key];
              });
              focus = this._formatQuickCreate(line,
                  second);
              focus.reconcileModelId = reconcileModelId;
              line.reconciliation_proposition.push(focus);
              this._computeReconcileModels(handle,
                  reconcileModelId);
          }
          line.createForm = _.pick(focus, this
              .quickCreateFields);
      ...
      _formatQuickCreate: function(line, values) {
          ...
          prop.product_id = this._formatNameGet(values
              .product_id);
          return prop;
      },
