...
def migrate(cr, version):
    ...
    # reappear on MyCompassion instead of only on the next hold/reactivate
    # cycle. Restricted to letters whose project/template is *currently* in
    # that state too, to keep this backfill narrowly scoped to the case this
    # ticket reported - a letter with hold evidence whose project has since
    # been reactivated is a separate, pre-existing concern (why didn't
    # reactivate_letters() clear it?), not something to fix here.
