    ...
    for report in reports:
        ...
        for opt in report.printer_options:
            if opt.option_key == 'InputSlot':
                report.printer_input_tray_id = opt
                remove.append((2, opt.id))
            elif opt.option_key == 'OutputBin':
                report.printer_output_tray_id = opt
                remove.append((2, opt.id))
        if remove:
            report.write({'printer_options_choices': remove})

    printers = env['printing.printer'].search([])
    for printer in printers:
        remove = []
        for opt in printer.printer_options_choices:
