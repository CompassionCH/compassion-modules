...
class contract_group(orm.Model):
    ...
    _inherit = 'simple.recurring.contract.group'
    ...
    def _setup_inv_data(self, cr, uid, con_gr, journal_ids, invoicer_id, context=None):
        ''' Inherit to add BVR ref '''
        inv_data = super(contract_group, self)._setup_inv_data(cr, uid, con_gr, journal_ids, invoicer_id, context)

        ref = ''
        if con_gr.bvr_reference:
            ref = con_gr.bvr_reference
        elif con_gr.payment_term_id and (_('LSV') in con_gr.payment_term_id.name  \
    ...
    def _setup_inv_line_data(self, cr, uid, contract_line, invoice_id, context=None):
        ...
        inv_line_data = super(contract_group, self)._setup_inv_line_data(cr, uid, contract_line, invoice_id, context)
