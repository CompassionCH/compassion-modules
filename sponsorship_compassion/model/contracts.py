...
class recurring_contract(orm.Model):
        ...
        if nb_invoices_canceled:
            ...
            for contract in self.browse(cr, uid, ids, context):
                ...
                cancel_ids = invoice_obj.search(cr, uid, [
                    ('state', '=', 'cancel'),
                    ('partner_id', '=', contract.partner_id.id)
                    ('date_invoice', '>=', today)], context=context)
                if cancel_ids:
                    inv_update_ids.add(cancel_ids)
                    invoice_obj.action_cancel_draft(cr, uid, cancel_ids)
                    self._update_invoice_lines(cr, uid, contract, cancel_ids, context)
    ...
    def _update_invoice_lines(self, cr, uid, contract, invoice_ids, context=None):
        ...
        for invoice in invoice_obj.browse(cr, uid, invoice_ids, context):
            ...
            invoice.write({
                'payment_term_id': contract.group_id.payment_term_id and
                                   contract.group_id.payment_term_id.id
                                   or False})
