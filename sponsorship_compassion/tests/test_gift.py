...
class TestGift(common.TransactionCase):
    def test_compute_date_gift_invoice(self):
        ...
        new_date = GenerateGiftWizard.compute_date_gift_invoice(gift_event_date, date.today())
        self.assertEqual(new_date, gift_event_date.replace(day=1) + relativedelta(months=-2))
        ...
        new_date = GenerateGiftWizard.compute_date_gift_invoice(gift_event_date, date.today() + relativedelta(days=36))
        self.assertEqual(new_date, gift_event_date.replace(day=1) + relativedelta(months=-2, years=1))
        ...
        new_date = GenerateGiftWizard.compute_date_gift_invoice(gift_event_date, date.today() + relativedelta(years=1))
        self.assertEqual(new_date, gift_event_date.replace(day=1) + relativedelta(months=-2, years=1))
