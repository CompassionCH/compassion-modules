...
class ProductProduct(models.Model):
    ...
    survival_sponsorship_sale = fields.Boolean(
        ...
        readonly=True,
        help="(Inherited from product template) The product can be sold "
        "as a survival sponsorship one",
    )
    survival_mom_name = fields.Char(
        "Name of the mom (com)", help="This field is used in the communications"
    )
    survival_child_name = fields.Char(
        "Name of the baby (com)", help="This field is used in the communications"
    )
    ...
    survival_slot_number = fields.Integer(
        compute="_compute_survival_slot_subscription",
        help="Number of places available for the survival sponsorship program "
        "in a given country",
        store=True,
    )
    survival_sponsorship_number = fields.Integer(
        compute="_compute_survival_slot_subscription",
        help="Number of active sponsorships for the survival sponsorship program "
        "in a given country",
        store=True,
    )
    alltime_survival_sponsorship_number = fields.Integer(
        compute="_compute_survival_slot_subscription",
        help="Number of sponsorships for the survival sponsorship program "
        "in a given country",
        store=True,
    )
    ...
    @api.depends(
        "survival_sponsorship_field_office_id",
        "survival_sponsorship_sale",
        "contract_line_ids",
    )
    def _compute_survival_slot_subscription(self):
        """
        Compute the number of slot available for that country survival program
        Compute the number of sponsorships that are active on this survival
        country program
        """
        for product in self:
            if (
                product.survival_sponsorship_sale
                and product.survival_sponsorship_field_office_id
            ):
                product.survival_slot_number = sum(
                    ...
                    .search(
                        [
                            ...
                            (
                                "field_office_id",
                                "=",
                                product.survival_sponsorship_field_office_id.id,
                            ),
                            ("type", "ilike", "survival"),
                        ]
                    )
                ...
                product.alltime_survival_sponsorship_number = sum(
                    product.contract_line_ids.mapped("quantity")
                )
                product.survival_sponsorship_number = sum(
                    product.contract_line_ids.filtered(
                        lambda c: c.contract_id.state in ["active", "waiting"]
                    ).mapped("quantity")
                )
                product.slot_used = (
                    product.survival_sponsorship_number / product.survival_slot_number
    ...
    def warn_admin(self):
        responsible_product_warn_user = (
            ...
            .sudo()
            .get_param("survival_sponsorship_warn_user_ids")
        )
        for product in self:
            if product.slot_used >= 80 and product.survival_sponsorship_sale:
                product.activity_schedule(
                    ...
                    note="Some intervention should be added for the country "
                    "or the sale should stop.",
