    ...
    def _compute_amount_on_hold(self):
        for intervention in self:
            intervention.amount_on_hold = (
                intervention.total_cost - intervention.remaining_amount_to_raise
            )

    def _compute_can_be_funded(self):
        for intervention in self:
            intervention.can_be_funded = (
                intervention.funding_status
                in ("Available", "Partially Held", "Partially Committed")
        ...
        for data in data_array:
            if "category_id" in data and "type" in data:
                ...
                category = category_obj.search(
                    [("name", "=", selected_category.name), ("type", "=", data["type"])]
                )
