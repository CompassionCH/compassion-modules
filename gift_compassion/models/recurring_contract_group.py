    ...
    def _cancel_gift_inv(self):
        ...
        for move_line in contract_not_dd.mapped("invoice_line_ids").filtered(
            ...
            if len(move.mapped("invoice_line_ids")) > 1:
                ...
                empty_move |= move
            # dynamic parts of the message are inserted after the translation marker
            # _(), which prevent name 'self' not defined error during term extraction.
            message_template = (
                "{} because the payment mode of the "
                "<a href=# data-oe-model={} data-oe-id={}>"
                "Payment option ({})</a> has been modified"
            )
            formatted_message = message_template.format(
                message, self._name, group_id.id, group_id.ref
            )
            ...
            conditional_message = (
                ...
                "The <a href=# data-oe-model={} data-oe-id={}>invoice ({})</a> "
                "has been cancelled."
                if "cancelled" in message
                else " has some lines that were unlinked."
            )
            # Now format the dynamic content into the message
            if "cancelled" in message:
                formatted_message = conditional_message.format(
                    move._name, move.id, move.name
                )
            else:
