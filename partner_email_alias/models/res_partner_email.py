...
class PartnerEmail(models.Model):
    ...
    _sql_constraints = [
        ("unique_email", "unique(email)", "This e-mail alias is already registered for a partner.")
    ]

    @api.constrains("email")
    def _unique_email(self):
        for alias in self:
            if self.env["res.partner"].search_count([("email_normalized", "=", alias.email)]):
                raise ValidationError("This email address is already set in one partner")
