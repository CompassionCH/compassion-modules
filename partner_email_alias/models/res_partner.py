...
class ResPartner(models.Model):
    ...
    email_alias_ids = fields.One2many(
        "res.partner.email", "partner_id", "Email aliases"
    )
    ...
    def get_alias(self, email):
        ...
        """
        self.ensure_one()
        return self.email_alias_ids.filtered(
            lambda a: a.email == email_normalize(email)
        )
            ...
            if email_normalized:
                partner = self.search(
                    [
                        ...
                        ("email_alias_ids.email", "=", email_normalized),
                    ],
                    limit=1,
                )
                if partner:
