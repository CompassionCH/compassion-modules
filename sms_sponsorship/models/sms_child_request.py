...
class SmsChildRequest(models.Model):
    ...
    def _compute_has_filter(self):
        for request in self:
            request.has_filter = (
                request.gender
                or request.min_age
                or request.field_office_id
                or (request.max_age and request.max_age != DEFAULT_MAX_AGE)
            )
    ...
    def create(self, vals):
        ...
        request.write(
            {
                "step1_url_id": self.env["link.tracker"]
                .sudo()
                .create(
                    {
        ...
        self.write(
            {
                ...
                "state": "step1",
                "step2_url_id": self.env["link.tracker"]
                .sudo()
                .create(
                    {
        ...
        try:
            ...
            result_action = (
                self.env["child.hold.wizard"]
                    .with_context(active_id=childpool_search.id, async_mode=False)
                    .create(
                    {
            ...
            if child_hold.state == "active":
                self.write(
                    {"child_id": child_hold.child_id.id, "state": "child_reserved"}
                )
    ...
    def check_child_parameters(self, child):
        ...
        country_match = (
            not self.field_office_id or self.field_office_id == child.field_office_id
        )
