...
class TranslationCompetence(models.Model):
    ...
    _sql_constraints = [
        (
            "unique_competence",
            "unique(source_language_id,dest_language_id)",
            "This competence already exists.",
        )
    ]
    ...
    def _compute_name(self):
        for competence in self:
            competence.name = (
                competence.source_language_id.name
                + " -> "
                + competence.dest_language_id.name
            )
    ...
    def get_translation_languages(self):
        ...
        languages = competences.mapped("source_language_id") | competences.mapped(
            "dest_language_id"
        )
...
class TranslationUserSkill(models.Model):
    ...
    translator_id = fields.Many2one(
        "translation.user", required=True, index=True, ondelete="cascade"
    )
    competence_id = fields.Many2one(
        "translation.competence", required=True, index=True, ondelete="cascade"
    )
