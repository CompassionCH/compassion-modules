...
export const TranslatorDAO = {
     ...
     */
    async find(orm, id) {
        const data = await orm.call("translation.user", "get_user_info", [[parseInt(id, 10)]]);
    ...
    async list(orm, params = {}) {
        ...
        const rawTranslators = await orm.call("translation.user", "list_users", [ids]);
    ...
    async registerSkills(orm, translatorId, competenceIds) {
        for (const skillId of competenceIds) {
            await orm.call("translation.user", "add_skill", [[translatorId], skillId]);
        }
        return true;
    },
