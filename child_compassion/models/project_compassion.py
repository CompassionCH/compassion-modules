...
class CompassionProject(models.Model):
    ...
    gps_latitude_obfuscated = fields.Float(compute="_compute_gps_obfuscated")
    gps_longitude_obfuscated = fields.Float(compute="_compute_gps_obfuscated")
    ...
    def _compute_reactivated_since_2_month(self):
        ...
        for project in self:
            ...
            has_reactivation = project.lifecycle_ids.filtered(
                lambda x: x.date
                and x.type == "Reactivation"
                and x.date >= two_months_ago
            )
