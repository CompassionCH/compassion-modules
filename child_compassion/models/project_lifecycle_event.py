...
class ProjectLifecycle(models.Model):
    ...
    def process_commkit(self, commkit_data):
        ...
        lifecycle_ids = list()
        for single_data in commkit_data.get('FCPLifecycleEventList',
                                            [commkit_data]):
            project = self.env['compassion.project'].search([
                ('fcp_id', '=', single_data['FCP_ID'])
            ])
            if not project:
                project.create({'fcp_id': single_data['FCP_ID']})
