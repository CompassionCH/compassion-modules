...
class MappedModel(models.AbstractModel):
    ...
    def _get_ir_translated_fields(self):
        """
        Returns a dict with key the full field name and value the
        corresponding ir.model.field record.
        """
        res = dict()
    ...
    def _fetch_translations(self, gmc_action):
        ...
        terms used in record description.
        """
        ...
        for lang in (
            ...
            for translated_record in self.with_context(lang=lang.lang_id.code):
                ...
                # We want to deal with all possible received values,
                ...
                for (
                    ...
                    if full_field_name.endswith(".value"):
                        # Exception for multipicklist records where
                        # the gmc_terms are stored in "name" field
                    ...
                    for i, english_val in enumerate(english_vals):
                        # Update term only if English is the same as the
                        # translated
    ...
    def _find_translation(
        ...
        """
        Find a translation inside received GMC data. It will traverse
        the data in order to extract the desired values.

        :param field_name: full field_name (in Odoo) used to lookup inside
                           gmc_data
        :param gmc_term: english value of the term expected from GMC
        :param gmc_action: GMC action providing the data
        :param gmc_data: Received GMC data in the desired language
                         (list of data)
        ...
        # Flatten the list of dictionaries in case the terms are spread into multiple
        # fields
        gmc_data = flatten(
