...
class MisSpnInfoSettings(models.TransientModel):
    ...
    mis_main_company_id = fields.Many2one(
        "res.company", string="Main reporting Company",
    )
    mis_child_sponsored_id = fields.Many2one(
        "account.account", "Child Sponsored account",
        domain=[("user_type_id.include_initial_balance", "=", True), ], #("company_id", "=", mis_main_company_id.id),
    )
    mis_contract_created_id = fields.Many2one(
        "account.account", "Contract Created account",
    ...
    @api.model
    def set_values(self):
        self.env["ir.config_parameter"].set_param("mis_contract_created_id", self.mis_contract_created_id.id)
        self.env["ir.config_parameter"].set_param("mis_child_sponsored_id", self.mis_child_sponsored_id.id)
        self.env["ir.config_parameter"].set_param("mis_main_company_id", self.mis_main_company_id.id)
        super(MisSpnInfoSettings, self).set_values()
...
class MisSpnInfo(models.Model):
    ...
    report_company_id = fields.Many2one("res.company", string="Reporting Company", readonly=True)
    ...
    def init(self):
        ...
        mis_main_company_id = self.env['res.company'].browse(param_obj.get_param("mis_main_company_id"))
        mis_child_sponsored_id = param_obj.get_param("mis_child_sponsored_id")
        script = opj(os.path.dirname(__file__), "spn_info.sql")
        currency = self.env['res.company'].search([("id", "=", mis_main_company_id.id)]).currency_id.id
        with open(script) as f:
            tools.drop_view_if_exists(self.env.cr, "mid_spn_info")
            sql = f.read() % (mis_child_sponsored_id, currency,
                                mis_main_company_id.id,
                                mis_child_sponsored_id, currency,
                                mis_main_company_id.id,
                                mis_contract_created_id, currency,
                                mis_main_company_id.id,
                                mis_contract_created_id, currency,
                                mis_main_company_id.id )
