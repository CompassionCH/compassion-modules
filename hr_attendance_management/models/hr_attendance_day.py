...
class HrAttendanceDay(models.Model):
    ...
    _sql_constraints = [
        (
            "unique_attendance_day",
            "unique(date, employee_id)",
            'This "Attendance day" already exists for this ' "employee",
        )
    ]
    ...
    cal_att_ids = fields.Many2many(
        "resource.calendar.attendance",
        store=True,
        compute="_compute_cal_att_ids",
        readonly=False,
    )
    working_day = fields.Char(compute="_compute_working_day", readonly=True, store=True)
    name = fields.Char(compute="_compute_name", store=True)

    # Leaves
    leave_ids = fields.Many2many("hr.leave", string="Leaves", readonly=False)
    # todo replace by employee_id.is_absent_totay
    in_leave = fields.Boolean("In leave", compute="_compute_in_leave", store=True)
    public_holiday_id = fields.Many2one(
        "hr.holidays.public.line", "Public holidays", readonly=False
    )

    # Due hours
    due_hours = fields.Float(
        "Due hours", compute="_compute_due_hours", readonly=True, store=True
    )

    # Attendances
    attendance_ids = fields.One2many(
        "hr.attendance", "attendance_day_id", "Attendances", readonly=True
    )

    has_change_day_request = fields.Boolean(
        compute="_compute_has_change_day_request",
        store=True,
        oldname="has_linked_change_day_request",
    )

    # Worked
    paid_hours = fields.Float(compute="_compute_paid_hours", store=True, readonly=True)
    ...
    # Break
    due_break_min = fields.Float("Minimum break due", compute="_compute_due_break")
    due_break_total = fields.Float("Total break due", compute="_compute_due_break")
    break_ids = fields.One2many(
        "hr.attendance.break", "attendance_day_id", "Breaks", readonly=True
    )
    break_total = fields.Float(
        "Total break", compute="_compute_break_total", store=True
    )
    rule_id = fields.Many2one(
        "hr.attendance.rules", "Rules", compute="_compute_rule_id", readonly=False
    )

    day_balance = fields.Float(
        "Day balance", compute="_compute_day_balance", store=True
    )
    ...
    def _compute_cal_att_ids(self):
        ...
        for att_day in self:
            ...
            current_cal_att = att_day.working_schedule_id.mapped(
                "attendance_ids"
            ).filtered(lambda a: int(a.dayofweek) == week_day)

            # Specific period
            att_schedule = current_cal_att.filtered(
                lambda r: r.date_from is not False
                and r.date_to is not False
                and r.date_from <= att_day.date <= r.date_to
            )
            ...
            if not att_schedule:
                att_schedule = current_cal_att.filtered(
                    lambda r: (
                        r.date_from and r.date_from <= att_day.date and not r.date_to
                    )
                    or (
    ...
    @api.multi
    @api.depends(
        "working_schedule_id", "leave_ids", "public_holiday_id", "cal_att_ids.due_hours"
    )
    ...
    def _compute_free_break_hours(self):
        for att_day in self:
            att_day.free_breaks_hours = sum(
                att_day.break_ids.filtered("is_offered").mapped("total_duration") or [0]
            )
    ...
    def _compute_break_total(self):
        for att_day in self:
            att_day.break_total = sum(att_day.break_ids.mapped("total_duration") or [0])
    ...
    def validate_extend_breaks(self):
        ...
        def extend_longest_break(extension_duration):
            ...
            att_breaks = att_day.break_ids.filtered(lambda r: not r.is_offered)
            ...
            att_break.write({"additional_duration": extension_duration})

        def compute_break_time_to_add(rule):
            breaks_total = sum(att_day.break_ids.mapped("total_duration") or [0])
            due_break_total = rule["due_break_total"]
            due_break_min_length = rule["due_break"]

            time_to_add = 0
            break_max = max(att_day.break_ids.mapped("total_duration") or [0])
            if break_max < due_break_min_length:
                # We want to extend an non-offered break to at least the
                # minimum value.
                break_max_non_offered = max(
                    att_day.break_ids.filtered(lambda b: not b.is_offered).mapped(
                        "total_duration"
                    )
                    or [0]
                )
        ...
        for att_day in self:
            ...
            if time_to_add != 0:
                ...
                if new_rule != rule:
                    ...
                    time_to_add = max(time_to_add, logged_hours - new_rule.time_to)
                if time_to_add != 0:
    ...
    def create(self, vals):
        ...
        co_ids = (
            self.env["hr.weekday.coefficient"]
                .search([("day_of_week", "=", week_day)])
                .filtered(lambda r: r.category_ids & rd.employee_id.category_ids)
        )
        rd.coefficient = co_ids[0].coefficient if co_ids else 1

        # check public holiday
        if self.env["hr.holidays.public"].is_public_holiday(rd.date, rd.employee_id.id):
            holidays_lines = self.env["hr.holidays.public"].get_holidays_list(
                att_date.year, rd.employee_id.id
            )
            rd.public_holiday_id = holidays_lines.filtered(lambda r: r.date == rd.date)
