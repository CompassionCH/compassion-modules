...
class HrChangeDayRequest(models.Model):
    ...
    user_id = fields.Many2one(
        "res.users", string="Manager", track_visibility="onchange", readonly=False
    )
    ...
    employee_id = fields.Many2one(
        "hr.employee", related="day1_id.employee_id", readonly=False
    )
    ...
    def create(self, vals):
        ...
        res = super().create(
            {"day1_id": day1_id.id, "day2_id": day2_id.id, "user_id": vals["user_id"]}
        )
    ...
    def _compute_name(self):
        for h in self:
            h.name = (
                f"{h.employee_id.display_name}, {h.date1}/{h.date2}, "
                f"{self.env['hr.employee'].convert_hour_to_time(h.forced)} "
                f"changed"
            )
