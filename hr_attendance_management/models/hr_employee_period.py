...
def get_surrounding_period(employee_periods, res):
    return employee_periods.filtered(
        lambda r: r.start_date < res.start_date and r.end_date > res.end_date
    )
...
class HrEmployeePeriod(models.Model):
    ...
    end_date_display = fields.Date(
        string="Last date of period", compute="_compute_display_of_end_date"
    )
    balance = fields.Float(string="Balance of period")
    final_balance = fields.Float(
        string="Total balance at end of period",
        compute="_compute_final_balance",
        readonly=True,
        store=True,
    )
    previous_period = fields.Many2one(
        "hr.employee.period", string="Previous period", readonly=True
    )
        ...
        for current_period in self:
            ...
            if origin == "compute":
                ...
                current_period.write(
                    {"balance": balance, "final_balance": final_balance}
                )
    ...
    def calculate_balance_and_final_balance(self):
        ...
        extra, lost = self.employee_id.past_balance_computation(
            start_date=start_date, end_date=end_date, existing_balance=previous_balance
        )
    ...
    def create(self, vals):
        ...
        if end_date and start_date and not origin == "create":
            ...
            previous_overlapping_period = get_previous_overlapping_period(
                employee_periods, res
            )
    ...
    def handle_previous_period(
        ...
        if (
            ...
            res.write({"previous_period": period.id})

            period.update_period()
            previous_period.update_period()
        else:
            res.write({"previous_period": previous_period.id})
    ...
    def handle_surrounding_period(self, surrounding_period, start_date, end_date, res):
        ...
        res.write({"previous_period": period1.id})
    ...
    def handle_previous_overlapping_period(
        ...
        res.write({"previous_period": previous_overlapping_period.id})
    ...
    def handle_next_overlapping_period(self, next_overlapping_period, end_date, res_id):
        ...
        next_overlapping_period.write(
            {"start_date": end_date, "previous_period": res_id}
        )
