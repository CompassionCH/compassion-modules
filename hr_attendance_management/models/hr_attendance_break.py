...
class HrAttendanceBreak(models.Model):
    ...
    employee_id = fields.Many2one(
        "hr.employee", "Employee", required=True, readonly=False
    )
    ...
    original_duration = fields.Float(compute="_compute_original_duration", store=True)
    ...
    def _compute_name(self):
        for rd in self:
            if rd.is_offered:
                ...
                start = fields.Datetime.context_timestamp(rd.employee_id, rd.start)
                stop = fields.Datetime.context_timestamp(rd.employee_id, rd.stop)
                rd.name = start.strftime("%H:%M") + " - " + stop.strftime("%H:%M")
